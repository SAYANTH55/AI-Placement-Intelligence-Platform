...
@router.get("/stats")
async def get_stats(db: Session = Depends(get_db), current_user: User = Depends(require_role(["admin"]))):
    try:
...
async def get_round_analytics(db: Session = Depends(get_db), current_user: User = Depends(require_role(["admin"]))):
    try:
...
async def get_drive_stats(db: Session = Depends(get_db), current_user: User = Depends(require_role(["admin"]))):
    try:
...
@router.get("/pr-stats")
async def get_pr_stats(db: Session = Depends(get_db), current_user: User = Depends(require_role(["admin"]))):
    try:
