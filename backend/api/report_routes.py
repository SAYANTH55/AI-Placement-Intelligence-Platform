...
async def generate_dossier(
    ...
    if body.user_id and current_user.role in ADMIN_ROLES:
    ...
    elif body.user_id and current_user.role not in ADMIN_ROLES:
        logger.warning(
            f"[report] Student {current_user.id} attempted admin override — denied."
        )
    ...
    # ── Check cached report ───────────────────────────────────────────────────
    cached_record: Optional[GeneratedReport] = (
