        ...
        <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-white">Application Management</h1>
                        <p className="text-xs text-secondary-muted mt-1">Manage applicants, update round results, and export data.</p>
                    </div>
                    <div className="flex gap-2">
                        {selectedDrive && (
                            <button onClick={handleExportCSV} className="glass-panel hover:bg-white/[0.04] text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2">
                                <Download size={14} /> Export CSV
                            </button>
                        )}
                        <select className="glass-search p-2.5 rounded-xl text-xs text-white outline-none focus:border-primary-accent min-w-[200px]"
            ...
            {selectedDrive && !loading && (
                <div className="flex flex-col md:flex-row gap-3 glass-panel shadow-card-depth p-4 rounded-2xl items-center">
                    <div className="flex items-center gap-2 glass-search rounded-xl px-3 flex-1 w-full">
                        ...
                        <input type="text" placeholder="Search by student name..." className="bg-transparent border-none outline-none text-xs text-white p-2.5 w-full placeholder-tertiary-muted"
                            value={filters.search} onChange={e => setFilters({...filters, search: e.target.value})} />
                    </div>
                    <select className="glass-search p-2.5 rounded-xl text-xs text-white outline-none w-full md:w-auto"
                ...
                {selectedApps.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            <select className="bg-black/30 border border-primary-accent/30 p-2 rounded-lg text-xs text-white outline-none flex-1 sm:w-48"
                    ...
                    <tbody className="divide-y divide-white/5">
                        {!selectedDrive ? (
                            ...
                            <motion.tr key={app.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.03 }}
                                className={`hover:bg-white/[0.05] transition-colors ${selectedApps.includes(app.id) ? 'bg-[#FF4D26]/10' : ''}`}>
                                ...
                                <td className="px-6 py-4">
                                    <button onClick={() => setStudentModalId(app.student_id)} className="text-left group">
                                        <p className="font-bold text-white group-hover:text-primary-accent transition-colors">{app.student_name}</p>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end items-center gap-2">
                                        {(app.status === 'Applied' || app.status === 'In Progress') ? (<>
                                            <select className="glass-search p-1.5 rounded-lg text-xs text-white outline-none min-w-[120px]"
                ...
                {studentModalId && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                            className="glass-panel shadow-card-depth border-white/5 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/20">
                                <div>
                                    <h3 className="text-xl font-black text-white">{studentProfile ? studentProfile.name : 'Loading...'}</h3>
                                    <p className="text-tertiary-muted text-xs font-bold">{studentProfile?.email} • Batch {studentProfile?.batch}</p>
                                </div>
                                <button onClick={() => setStudentModalId(null)} className="p-2 hover:bg-white/5 rounded-full text-secondary-muted hover:text-white transition-colors"><X size={20} /></button>
                            </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-2">
                                                {studentProfile.profile_data?.skills?.slice(0,4).map(s => <span key={s} className="px-2 py-0.5 bg-white/5 text-[10px] font-bold rounded-md text-white">{s}</span>)}
                                                {(studentProfile.profile_data?.skills?.length || 0) > 4 && <span className="px-2 py-0.5 bg-white/5 text-[10px] font-bold rounded-md text-white">+{studentProfile.profile_data.skills.length - 4}</span>}
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            {studentProfile.applications.map(app => (
                                                <div key={app.application_id} className="border border-white/5 rounded-2xl p-4 bg-white/[0.02]">
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div><p className="font-black text-sm text-white">{app.company_name}</p><p className="text-secondary-muted text-xs font-bold">{app.role}</p></div>
                                                    ...
                                                    <div className="relative pl-4 border-l border-white/5 space-y-4">
                                                        {app.rounds.map(r => (
                                                            <div key={r.round_id} className="relative">
                                                                ...
                                                                <p className="text-xs font-bold text-white">{r.round_name}</p>
