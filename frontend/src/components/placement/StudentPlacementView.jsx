...
const StudentPlacementView = () => {
    ...
    return (
        <div className="p-8 bg-[#F4EFE4] min-h-screen text-[#1B2A4A] w-full">
            <h1 className="text-[#1B2A4A] mb-4 text-3xl font-black tracking-tight">Placement Opportunities</h1>
            <p className="text-sm text-[#888888] mb-8 font-medium">AI Readiness Score indicates your likelihood of passing the first rounds for these roles.</p>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full max-w-6xl mb-8">
                {drives.length > 0 ? drives.map((drive, idx) => (
                    <div key={idx} className={`bg-[#08080A] border rounded-[1.5rem] p-6 shadow-[0_0_20px_rgba(27,42,74,0.03)] transition-all ${cardBorderColor(drive.status)}`}>
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="text-xl font-black text-[#1B2A4A]">{drive.company_name}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <p className="text-[10px] uppercase font-bold tracking-widest text-[#1B2A4A]">{drive.role}</p>
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <p className="text-xs text-[#888888] font-semibold flex items-center gap-1">Deadline: {drive.deadline ? new Date(drive.deadline).toLocaleDateString() : 'Ongoing'}</p>
                            {drive.ctc && <span className="text-xs font-black text-green-400">{drive.ctc}</span>}
                        </div>
                        
                        {drive.description && (
                            <p className="text-xs text-[#888888555] mb-6 line-clamp-2 italic">"{drive.description}"</p>
                        )}
                        
                        <button className="w-full bg-[#1B2A4A] hover:bg-[#9ECCFA] text-white font-black text-sm py-3 rounded-xl transition-all duration-200">
                            Apply Now
                        </button>
                    </div>
                )) : (
                    <div className="bg-[#08080A] border border-[#181818] rounded-[1.5rem] p-12 shadow-[0_0_20px_rgba(27,42,74,0.03)] col-span-full text-center">
                        <p className="text-[#888888] text-sm">No active placement drives found.</p>
                    </div>
                )}
            </div>

            <div className="bg-[#08080A] border border-[#181818] rounded-[1.5rem] p-6 w-full max-w-6xl">
                <h3 className="text-lg font-black text-[#1B2A4A] mb-2">My Application Tracking</h3>
                <p className="text-sm text-[#888888]">You have no active applications.</p>
            </div>
        </div>
    );
};
