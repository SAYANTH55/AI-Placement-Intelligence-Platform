...
export default function Features() {
  ...
  return (
    <div className="min-h-screen bg-[#F4EFE4] text-gray-900 py-24 font-sans flex flex-col justify-center overflow-hidden relative">
      {/* Grid background */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-[#1B2A4A]/10 blur-[100px] rounded-[100%] pointer-events-none" />
        ...
        >
          <span className="inline-flex items-center gap-2 bg-[#1B2A4A]/10 text-[#1B2A4A] text-xs font-bold uppercase tracking-widest px-5 py-2.5 rounded-full mb-6 border border-[#1B2A4A]/30 shadow-sm">
            <CornerRightDown size={14} className="text-[#1B2A4A]" /> Pipeline Architecture
          </span>
          <h1 className="text-4xl sm:text-6xl font-black text-[#1B2A4A] mb-6 tracking-tight">
            Data-Driven <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1B2A4A] to-[#1B2A4A]">Intelligence Core</span>
          </h1>
        ...
        <div className="w-full overflow-x-auto pb-10 hide-scrollbar px-4 relative z-10 perspective-1000">
          ...
          <div 
            className="p-3 sm:p-6 rounded-[2.5rem] bg-[#FFFFFF]/60 border border-[#C9C2AF] shadow-[0_40px_100px_-20px_rgba(27,42,74,0.2)] relative max-w-[1050px] mx-auto min-w-[900px]"
            style={{ backdropFilter: 'blur(12px)', transformStyle: "preserve-3d" }}
          >
            {/* The Actual Diagram Container */}
            <div className="relative w-full aspect-square md:aspect-[16/11] lg:aspect-[16/10] bg-[#F4EFE4] rounded-[2rem] border border-[#C9C2AF] shadow-[inset_0_2px_20px_rgba(0,0,0,0.3)] overflow-hidden">
                    ...
                    <React.Fragment key={`dynamic-lines-${i}`}>
                      ...
                      {isTracerActive && (
                        <motion.path
                          ...
                          strokeWidth="4"
                          strokeLinecap="round"
                          style={{ filter: 'drop-shadow(0px 0px 8px rgba(27,42,74,0.8))' }}
              ...
              <div className="absolute inset-0 w-full h-full pointer-events-none z-10">
                ...
                {features.map((f, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* Floating Node Label */}
                      <div className={`absolute -top-10 font-black uppercase tracking-[0.15em] text-[10px] md:text-[11px] whitespace-nowrap transition-all duration-300 ${isNodeFocused ? 'text-[#1B2A4A] scale-110' : 'text-gray-400'}`}>
                        {f.title.replace(' ', '\n').split('\n')[0]}
                      </div>
                      ...
                      <motion.div 
                        ...
                        animate={{ 
                          ...
                          backgroundColor: isNodeFocused ? '#1B2A4A' : 'rgba(27,42,74,0.08)',
                          boxShadow: isNodeFocused ? '0 10px 25px -5px rgba(27,42,74,0.4), 0 8px 10px -6px rgba(27,42,74,0.1)' : '0 1px 3px 0 rgba(0, 0, 0, 0.05)'
                        }}
                        className={`w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center border-2 ${isNodeFocused ? 'border-[#1B2A4A]' : 'border-[#1B2A4A]/20 group-hover:border-[#1B2A4A]/40'}`}
                      >
                        {React.cloneElement(f.icon, { 
                          size: 24, 
                          className: `transition-colors duration-300 ${isNodeFocused ? "text-white" : "text-[#1B2A4A]/40 group-hover:text-[#1B2A4A]"}` 
                        })}
                      </motion.div>
                      ...
                      {isNodeFocused && (
                        <motion.div 
                          ...
                          className="absolute w-16 h-16 rounded-full border border-[#1B2A4A] pointer-events-none"
                        />
                      )}
                    </div>
                  );
                ...
                >
                  <motion.div 
                    className="relative w-full bg-[#FFFFFF] p-8 sm:p-10 rounded-[2.5rem] border border-[#C9C2AF] shadow-[0_20px_50px_-15px_rgba(27,42,74,0.2)] flex flex-col items-center text-center overflow-hidden shadow-[0_0_30px_rgba(27,42,74,0.1)] animate-breathing"
                    layoutId="hubContainer"
                  >
                    {/* Notch where lines connect */}
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-1.5 bg-[#1B2A4A] shadow-[0_0_15px_#1B2A4A]" />
                    ...
                    <div className="w-full h-32 flex flex-col justify-center items-center mb-6">
                      <AnimatePresence mode="wait">
                        ...
                        >
                          <span className="text-[#1B2A4A] font-black tracking-[0.2em] text-xs sm:text-sm mb-4 uppercase inline-block">
                             [ {activeFeature.title} ]
                          </span>
                          <p className="text-[#888888555] font-medium text-sm sm:text-base leading-relaxed">
                             {activeFeature.desc}
                          </p>
                        </motion.div>
                      </AnimatePresence>
                    ...
                    <button
                       ...
                       className="w-full inline-flex justify-center items-center gap-3 bg-[#1B2A4A] text-white font-bold tracking-wide px-8 py-4 rounded-2xl hover:bg-[#9ECCFA] hover:shadow-[0_10px_20px_rgba(27,42,74,0.3)] hover:scale-[1.02] active:scale-95 transition-all duration-300"
                     >
                       Initiate Extraction
                       <ArrowRight size={18} />
                     </button>
                  ...
                  <div className="mt-6 flex flex-col items-center opacity-60">
                    ...
                    <div className="w-10 h-10 rounded-full border-2 border-[#1B2A4A]/30 bg-[#FFFFFF] flex items-center justify-center text-[#1B2A4A] font-black font-mono text-sm shadow-sm transition-opacity duration-300">
                      0{activeIndex + 1}
                    </div>
                  </div>
                </div>
